...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  let currentStock = 0; // lưu số lượng tồn kho của size đang chọn
  ...
  const stockInfo = document.getElementById("stockInfo"); // hiển thị tồn kho
  ...
  async function loadProduct() {
    try {
      ...
      if (p.anhSP) productImage.src = `/Asset/${p.anhSP}`;
        ...
        p.sizes.forEach((s) => {
          ...
          btn.addEventListener("click", () => {
            ...
            if (stockInfo) {
              ...
              stockInfo.style.color = currentStock < 5 ? "red" : "#555";
            }
  ...
  if (addToCartBtn) {
    addToCartBtn.addEventListener("click", async () => {
      ...
      if (qty > currentStock) {
        alert(`Sản phẩm chỉ còn ${currentStock} cái trong kho!`);
        return;
      }
      ...
      try {
        ...
        } else alert(result.message || "Lỗi khi thêm vào giỏ");
      } catch (err) {
        console.error(err);
  ...
  if (buyNowBtn) {
    buyNowBtn.addEventListener("click", () => {
      ...
      // Chuyển tới checkout.html kèm thông tin qua URL
      const url = `/html/checkout.html?maSP=${productId}&maSize=${selectedSizeId}&soLuong=${qty}`;
